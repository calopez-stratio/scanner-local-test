...
private[impl] case class Surname() extends EvaluationFunction {
  ...
  def evaluateDatum(datum: Any)(implicit metadataSet: Set[ColumnMetadata], scannerType: ScannerType): Confidence = {
    ...
    NLPModel.getPrediction(strDatum, System.identityHashCode(this)) match {
      case Some(predictions) if predictions.contains(Prediction.surname) && !strDatum.containsNumbersOrEspecialChars =>
        HIGH_CONFIDENCE
...
private[impl] object Surname extends NoParamDSL[Surname] {

  override val keyword: String = "surname_nlp"
