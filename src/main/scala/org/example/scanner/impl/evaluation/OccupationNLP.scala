...
private[impl] case class OccupationNLP() extends EvaluationFunction {
  ...
  def evaluateDatum(datum: Any)(implicit metadataSet: Set[ColumnMetadata], scannerType: ScannerType): Confidence = {
    ...
    NLPModel.getPrediction(strDatum, System.identityHashCode(this)) match {
      case Some(predictions) if predictions.contains(Prediction.occupation) && !strDatum.containsNumbers =>
        HIGH_CONFIDENCE
...
private[impl] object OccupationNLP extends NoParamDSL[OccupationNLP] {

  override val keyword: String = "occupation_nlp"
