...
private[impl] case class RewardPlan() extends EvaluationFunction {
  ...
  def evaluateDatum(datum: Any)(implicit metadataSet: Set[ColumnMetadata], scannerType: ScannerType): Confidence = {
    ...
    NLPModel.getPrediction(strDatum, System.identityHashCode(this)) match {
      case Some(predictions)
        if predictions.contains(Prediction.rewardPlan) && (strDatum.length == 12 || strDatum.length == 7) =>
        HIGH_CONFIDENCE
      case Some(predictions) if predictions.contains(Prediction.rewardPlan) => MEDIUM_CONFIDENCE
...
private[impl] object RewardPlan extends NoParamDSL[RewardPlan] {

  override val keyword: String = "reward_plan_nlp"
